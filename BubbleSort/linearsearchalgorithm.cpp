//TEST SCORE ANALYSIS
//This program reads in test scores from a file,performs various calculations and allows the
//user to search for students using LSA.
//Files included:
...
int main(){
    ...
    <<"\n\nTotal for Student #8 is:"<<totalOneStudent(scores,NUM_STDS,7)
    <<"\n\nAverage for Test #5 is:"<<averageOneTest(scores,NUM_STDS,4)
    <<"\n\nAverage for Test #8 is:"<<averageOneTest(scores,NUM_STDS,7)
    <<"\n\nAverage for Student #5 is:"<<averageOneStudent(scores,NUM_STDS,4)
    <<"\n\nAveragefor Student #9 is:"<<averageOneStudent(scores,NUM_STDS,8)
    <<"\n\nHighest test score for Student #2 is:"<<highestOneStudent(scores,NUM_STDS,1)
    <<"\n\n The top Student:"<<topStudent(scores,NUM_STDS)<<endl;

    cout<<"/nFind if at least one student has an average score greater\n"
...
//linearsearch algorithm(time complexity:O(n))
bool searchStudents(const int scores[][NUM_TESTS],int numStds,double average){
    int studentNumber=0;
    bool found=false;
